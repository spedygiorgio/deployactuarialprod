...
\makeatother
\makeatletter
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[boxrule=0pt, breakable, colback={codebgcolor}, frame hidden, borderline west={3pt}{0pt}{shadecolor}, enhanced, sharp corners]}{\end{tcolorbox}}\fi
\makeatother
...
\title{Deploying models in production}
...
\phantomsection\label{mlops-enhancing-the-ml-model-lifecycle}
\begin{itemize}
\item
...
\item
  ...
  engineers for models that are more performing, scalable, and
  maintainable.
\item
...
\phantomsection\label{mlops-instruments}
\begin{itemize}
\tightlist
\item
...
\phantomsection\label{project-presentation}
\begin{itemize}
\tightlist
\item
...
\phantomsection\label{general-recommendations}
\begin{block}{AI tools}
\phantomsection\label{ai-tools}
\begin{itemize}
\tightlist
\item
...
\phantomsection\label{data-exploration-and-understanding}
\begin{itemize}
\tightlist
\item
...
\phantomsection\label{production-mindset}
\begin{itemize}
\tightlist
\item
...
\phantomsection\label{communication-and-documentation}
\begin{itemize}
\tightlist
\item
...
\phantomsection\label{python-pipelines}
\begin{block}{Python set up}
\phantomsection\label{python-set-up}
\begin{block}{Why Python}
\phantomsection\label{why-python}
\begin{itemize}
\item
...
{\centering \includegraphics[width=0.4\linewidth,height=\textheight,keepaspectratio]{assets/Python.svg.png}
...
\begin{frame}
\begin{block}{Python environment}
\phantomsection\label{python-environment}
\begin{itemize}
\item
  It is always advisable to create and environment in the project
  working directory
\item
  The environment allows to set up the necessary libraries and tools to
  make the workflow reproducible and isolated
\item
  Tools like \emph{uv} and \emph{conda} make easier the creation of
  python environment
\end{itemize}
\end{block}
\end{frame}
...
\begin{frame}
\begin{block}{Python pipelines}
\phantomsection\label{python-pipelines-1}
...
\phantomsection\label{running-the-pipeline}
\begin{itemize}
\tightlist
\item
...
\phantomsection\label{streamlit-walkthrough}
\begin{block}{Why streamlit?}
\phantomsection\label{why-streamlit}
\begin{itemize}
\tightlist
\item
...
\phantomsection\label{streamlit-code}
\begin{itemize}
\tightlist
\item
...
\phantomsection\label{running-the-app}
\begin{itemize}
\tightlist
\item
  The app is run with \texttt{streamlit\ run\ quote-page.py}
\item
  An initial section allows to input the policyholder's data
...
\phantomsection\label{fastapi-walkthrough}
\begin{block}{Why FastAPI?}
\phantomsection\label{why-fastapi}
\begin{itemize}
\tightlist
\item
...
\phantomsection\label{structure-of-a-fastapi-app}
\begin{itemize}
\tightlist
\item
...
\phantomsection\label{key-components-of-the-insurance-prediction-api}
\begin{block}{1. Input Schema}
\phantomsection\label{input-schema}
\begin{itemize}
\tightlist
\item
...
\phantomsection\label{output-schema}
\begin{itemize}
\tightlist
\item
...
\phantomsection\label{startup-event}
\begin{itemize}
\tightlist
\item
...
\phantomsection\label{prediction-endpoint}
\begin{itemize}
\tightlist
\item
...
\phantomsection\label{fastapi-in-action}
\begin{itemize}
\tightlist
\item
...
\begin{frame}[fragile]
\begin{block}{HTTP Status Codes to Discuss}
\phantomsection\label{http-status-codes-to-discuss}
\begin{itemize}
\tightlist
\item
  \textbf{2xx (Success)}: request processed correctly.

  \begin{itemize}
  \tightlist
  \item
    \texttt{200\ OK}: successful \texttt{/predict/}, \texttt{/health},
    \texttt{/ready}.
  \end{itemize}
\item
  \textbf{4xx (Client errors)}: input or request issue.
  ...
  \item
    \texttt{401\ Unauthorized}: authentication missing/invalid.
  \item
    \texttt{403\ Forbidden}: authenticated but not allowed.
  \item
    \texttt{404\ Not\ Found}: endpoint does not exist.
  \item
    \texttt{422\ Unprocessable\ Entity}: validation failed (common with
    Pydantic).
  \end{itemize}
\item
  \textbf{5xx (Server errors)}: backend issue.

  \begin{itemize}
  \tightlist
  \item
    \texttt{500\ Internal\ Server\ Error}: unhandled exception in API
    logic.
  \item
    \texttt{502\ Bad\ Gateway}: upstream/proxy cannot reach a healthy
    app.
  \item
    \texttt{503\ Service\ Unavailable}: service running but not ready
    (e.g., models not loaded).
  \item
    \texttt{504\ Gateway\ Timeout}: upstream did not respond in time.
  \end{itemize}
\end{itemize}
\end{block}
\end{frame}
...
\begin{frame}{Docker}
\phantomsection\label{docker}
\begin{block}{Why Docker?}
\phantomsection\label{why-docker}
\begin{itemize}
\tightlist
\item
...
\phantomsection\label{dockerfile-overview}
\begin{itemize}
\tightlist
\item
...
\phantomsection\label{docker-commands}
\begin{itemize}
\tightlist
\item
