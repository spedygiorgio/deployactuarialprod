...
-   An initial section allows to input the policyholder's data (pre-defined values exists)
...
### HTTP Status Codes to Discuss

-   **2xx (Success)**: request processed correctly.
    -   `200 OK`: successful `/predict/`, `/health`, `/ready`.
-   **4xx (Client errors)**: input or request issue.
...
-   **5xx (Server errors)**: backend issue.
    -   `500 Internal Server Error`: unhandled exception in API logic.
    -   `502 Bad Gateway`: upstream/proxy cannot reach a healthy app.
    -   `503 Service Unavailable`: service running but not ready (e.g., models not loaded).
